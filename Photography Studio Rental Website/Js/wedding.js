...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (heroImg) {
      document.querySelector('.cinematic-frame').addEventListener('mousemove', (e) => {
        const { left, top, width, height } = e.currentTarget.getBoundingClientRect();
        const x = (e.clientX - left) / width - 0.5;
        const y = (e.clientY - top) / height - 0.5;
        
        heroImg.style.transform = `perspective(1000px) rotateY(${x * 5}deg) rotateX(${y * -5}deg) scale(1.02)`;
      });

      document.querySelector('.cinematic-frame').addEventListener('mouseleave', () => {
