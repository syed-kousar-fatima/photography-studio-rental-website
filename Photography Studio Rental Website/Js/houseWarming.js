...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const heroVisual = document.querySelector('.home-visual-frame');
    const welcomeCard = document.querySelector('.floating-welcome-card');
    
    if (heroVisual && welcomeCard) {
      heroVisual.addEventListener('mousemove', (e) => {
        const { left, top, width, height } = heroVisual.getBoundingClientRect();
        const x = (e.clientX - left) / width - 0.5;
        const y = (e.clientY - top) / height - 0.5;
        
        welcomeCard.style.transform = `translate(${x * 40}px, ${y * 40}px)`;
      });

      heroVisual.addEventListener('mouseleave', () => {
        welcomeCard.style.transform = `translate(0, 0)`;
      });
    }
  });
